...
import "./libs/ERC20/SafeERC20.sol";
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of EASYs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accEASYPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accEASYPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Update the given pool's easy allocation point. Can only be called by the owner.
    ...
    // View function to see pending EASYs on frontend.
    function pendingEASY(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            accEASYPerShare = accEASYPerShare.add(easyReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accEASYPerShare).div(1e12).sub(user.rewardDebt);
    }
